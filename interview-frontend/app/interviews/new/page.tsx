...
export default function NewInterviewPage() {
  ...
  const [prompt, setPrompt] = useState(DEFAULT_PROMPT_UR);
  const [questions, setQuestions] = useState<Question[]>([
    { id: "1", text: "", type: "open-ended", order: 1 }
  ]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  ...
  const addQuestion = () => {
    const newId = String(questions.length + 1);
    setQuestions(prev => [...prev, {
      id: newId, text: "", type: "open-ended", order: prev.length + 1
    }]);
  };
  ...
  const updateQuestionType = (index: number, type: "open-ended" | "mcq") => {
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {(["open-ended", "mcq"] as const).map(type => (
                    <button
                      key={type}
                      ...
                      style={{
                        ...
                        cursor: "pointer",
                        transition: "all 0.2s",
                        background: q.type === type
                          ? type === "mcq"
                            ? "rgba(168,85,247,0.25)"
                            : "rgba(6,182,212,0.2)"
                          : "transparent",
                        borderColor: q.type === type
                          ? type === "mcq" ? "#a855f7" : "#06b6d4"
                          : "rgba(255,255,255,0.15)",
                        color: q.type === type
                          ? type === "mcq" ? "#d8b4fe" : "#67e8f9"
